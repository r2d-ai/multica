...
Three tables get created: `github_installation`, `github_pull_request`, `issue_pull_request`. Migration `128_github_pr_activity` adds `github_pr_activity` for dedupe and thread mapping. They cascade-delete with their workspace, so removing a workspace cleans them up automatically.
